...
rawRows.forEach((row, index) => {
  ...
  const cleanDesc = body.replace(/<[^>]*>?/gm, '').replace(/\s+/g, ' ').trim().substring(0, 150);
...
fs.writeFileSync(path.join(outDir, 'importedProducts.json'), JSON.stringify(parsedProducts, null, 2), 'utf8');
console.log(`Generated src/data/importedProducts.json with ${parsedProducts.length} items.`);
